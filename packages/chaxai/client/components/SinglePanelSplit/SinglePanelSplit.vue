...
const props = defineProps<{ handler: SinglePanelSplitHandler,extraStyle?: {[key:string]:string}  }>()
const handler = computed(() => props.handler)
const controller = ref<HTMLDivElement | null>(null)

const siderStyle = computed(()=>{
  console.warn(handler.value.siderBarWitdh)
  if(handler.value.siderBarWitdh){
    return {minWidth:handler.value.siderBarWitdh}
  }else{
    return {}
  }
})
...
const extraStyle = computed(() => {
  if (handler.value.isHorizontal()) {
    return { width: '0' ,...props.extraStyle??{}}
  } else {
    return { height: '0',...props.extraStyle??{} }
  }
})
...
<template>
  <div
    ...
    :class="{
      'panel-split': true,
      'panel-split--draging': handler.isDragging,
