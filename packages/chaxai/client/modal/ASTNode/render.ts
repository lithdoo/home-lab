...
export const defaultRendererKeyType = (type: MDNodeType, node: any) => {
    return null
}
...
export class CacheRootViewNode<Ctx> extends RootViewNode<Ctx> {
    ...
    constructor(
        ...
        const prefix = ''
        super(renderer, ctx, prefix + content)
        this.prefix = prefix
    }
    ...
    update(content: string): void {
        const newAst = parseMarkdownToAST(this.prefix + content)
        const oldAst = this.ast

        this.renderChildrenWithOld(true, newAst.children, oldAst.children)
        this.ast = newAst
    }
    ...
    private renderChildrenWithOld(isLast: boolean, list: MDNode[], oldList: MDNode[], parent?: MDViewNode) {
        const children = list.map((node, idx) => {
            ...
            if (RootViewNode.nodeIsParent(node) && viewNode) {
                ...
                this.renderChildrenWithOld(curIsLast, node.children as any, oldChildren, viewNode)
            }
            return viewNode
        }).filter(v => !!v)
