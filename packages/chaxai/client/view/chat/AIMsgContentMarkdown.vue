...
import { CacheRootViewNode } from '../../modal/ASTNode/render';
...
const root = new CacheRootViewNode(
    prop.chat.markdown.renderer,
    prop.chat.markdown.keyType,
    prop.chat
)
...
const load = async () => {
    if (!prop.chat.msgbox.msgSSE[prop.msg.msgId]) {
        const content = await prop.chat.msgbox.content(prop.msg.msgId)
        if (!content.trim()) {
            errorMsg.value = '生成错误'
        } else {
            root.update(content)
        ...
        if (prop.chat.msgbox.msgSSE[prop.msg.msgId].total) {
            showLoading.value = false
        }
    }
    emitter('finish')
...
const sse = computed(() => prop.chat.msgbox.msgSSE[prop.msg.msgId]?.total ?? '')

watch(sse, () => {
    if (!sse.value) return
    if (showLoading.value) showLoading.value = false
    const content = sse.value
    prop.checkScrollBottom(() => { root.update(content) }, true)
})
